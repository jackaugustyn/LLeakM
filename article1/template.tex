...
Interactive LLM assistants typically deliver responses incrementally, allowing users to see generated text almost immediately rather than waiting for a complete answer. A common implementation relies on Server-Sent Events (SSE), where each generated token is wrapped as an application-layer event of the form \texttt{data: \textless token\textgreater\textbackslash n\textbackslash n}. Transport encryption conceals event contents, but it does not automatically conceal event sizes. The present study does not recover those sizes from TLS, HTTP/2, or QUIC records. Instead, it records SSE-equivalent application events directly from an instrumented service, with exactly one event per generated token, in order to isolate the information carried by token-granular length sequences.
...
The evaluated models are listed in Table~\ref{tab:model_metadata}. Six are instruction- or chat-tuned; Phi-1.5 is a base model retained as a stress test~\cite{qwen25report,llama3report,llama32blog,gemma2report,phi15report,tinyllama2024,phi3report}. A common token budget does not imply a common amount of generated text across tokenizer families. Table~\ref{tab:length_covariates} reports response-length covariates computed from the 2100 publication samples. Truncation at the 96-token cap ranged from 95.0\% (Qwen2.5-1.5B-Instruct) to 100\% (four models), so the results primarily characterize reconstruction of truncated early-response content.
...
\begin{table}[ht]
\centering
\caption{Response-length covariates for the standardized subset ($N=300$ per model). Truncation is the percentage of samples with \texttt{token\_count}$\geq 96$.}
...
\midrule
Qwen2.5-1.5B-Instruct & 92.2 & 464 & 5.03 & 95.0 \\
Qwen2.5-3B-Instruct & 96.0 & 482 & 5.03 & 100.0 \\
Llama-3.2-3B-Instruct & 94.7 & 463 & 4.88 & 96.3 \\
Gemma-2-2B-it & 96.0 & 435 & 4.53 & 100.0 \\
Phi-1.5 & 96.0 & 497 & 5.17 & 99.7 \\
TinyLlama-1.1B-Chat-v1.0 & 96.0 & 331 & 3.45 & 100.0 \\
Phi-3.5-mini-instruct & 96.0 & 342 & 3.57 & 100.0 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
The experiment directory contained 20 candidate execution folders, of which 14 were artifact-complete and 6 were incomplete placeholders. The complete runs contained 4050 samples in total. The prespecified standardized subset used for primary comparison covered seven models and 2100 samples. Each included model contributed exactly 300 samples. The corresponding artifact directories are listed in Appendix~\ref{app:runs}.
...
The quality of reconstruction varied substantially between the evaluated models. The two Qwen2.5 variants consistently achieved the highest lexical recovery scores, followed by Phi-1.5 and Llama-3.2-3B-Instruct. Gemma-2-2B-it occupied an intermediate position, while Phi-3.5-mini-instruct and TinyLlama-1.1B-Chat-v1.0 exhibited only limited reconstructability in the evaluated pipeline. These differences suggest that the relationship between length patterns and generated content is not uniform across model families. Although Phi-1.5 achieved relatively strong results, it should be interpreted separately because it is a base model rather than an instruction-tuned assistant. Consequently, its performance does not imply that base models are inherently more vulnerable than instruction-following systems.
...
microsoft/phi-1\_5 & base & 0.2747 & 0.7426 & 0.2503 & 0.1891 & 4.333\% \\
...
Gemma-2-2B-it & 300/300 & 0.1798 & 0.7764 & 0.1719 & 0.1269 & 0.000\% \\
\bottomrule
\end{tabular}%
}
\end{table}
...
All seven standardized runs retain a topic label on every sample. The present topic-level analysis is nevertheless descriptive: no multiplicity-corrected inference was performed over model--topic combinations. Selected topic-level summaries are shown in Table~\ref{tab:topic_sensitivity}. For Llama-3.2-3B-Instruct, the highest reported topic mean was for \emph{employment} ($\phi=0.3373$), and the lowest was for \emph{safety\_security} ($\phi=0.2053$). Gemma-2-2B-it had the highest reported mean for \emph{beliefs\_values} and the lowest for \emph{financial\_status}. Phi-3.5-mini-instruct remained low across its reported topic extrema.

These descriptive observations suggest that reconstruction scores may depend on the linguistic distribution of generated content. They do not support confirmatory topic-level claims.
...
Reconstruction outcomes differed across the evaluated models under identical collection and reconstruction settings. This finding has practical implications only after token-granular application-event lengths are available. A service that encrypts SSE payloads at the transport layer is not thereby shown to be safe against reconstruction, but neither do the present measurements demonstrate that those lengths can be recovered from TLS, HTTP/2, or QUIC records.
...
\section{Conclusions}
This study examined the extent to which token-length information can reveal characteristics of generated content in streaming LLM systems. Using a Weiss-style reconstruction pipeline and an artifact-validated dataset of SSE-equivalent application-event lengths, we observed measurable lexical leakage across a diverse set of open-weight models, with reconstruction performance varying substantially between model families.
...
TinyLlama-1.1B-Chat-v1.0 & \texttt{run\_20260325\_153010\_tinyllama\_1\_1b} \\
