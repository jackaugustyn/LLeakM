...
# Standardized comparison: N=300, max_new_tokens=512, sps=3, max_sentences=0.
    ...
    with path.open(encoding="utf-8") as f:
        for line in f:
            ...
            rows[o["idx"]] = {
                ...
                "response_complete": bool(o.get("response_complete")),
...
def main() -> None:
    ...
    for label, pretty, run_dir in RUNS:
        ...
        complete_mask = np.array([bool(rows[i]["response_complete"]) for i in common_sorted])
        n_eos = int(complete_mask.sum())
        eos_pct = 100.0 * n_eos / len(phi)
        trunc_pct = 100.0 * float(np.mean([
            1.0 if rows[i]["token_count"] >= MAX_NEW_TOKENS else 0.0 for i in common_sorted
        ]))
        if n_eos:
            phi_c = phi[complete_mask]
            k_c = int((phi_c > ASR_THRESHOLD).sum())
            phi_cm, phi_clo, phi_chi = bootstrap_ci_mean(phi_c, rng)
            asr_cp, asr_clo, asr_chi = wilson_ci(k_c, n_eos)
            complete_only = {
                "n": n_eos,
                "phi_mean": phi_cm, "phi_ci": [phi_clo, phi_chi],
                "asr": 100 * asr_cp,
                "asr_ci": [100 * asr_clo, 100 * asr_chi],
                "asr_success": k_c,
            }
        else:
            complete_only = {
                "n": 0, "phi_mean": None, "phi_ci": [None, None],
                "asr": None, "asr_ci": [None, None], "asr_success": 0,
            }
        ...
        model_stats.append({
            ...
            "asr_success": k,
            "n_eos": n_eos, "eos_percent": eos_pct, "trunc_percent": trunc_pct,
...
def _write_tables(model_stats: list[dict], pairs: list[dict]) -> None:
    ...
    for m in sorted(model_stats, key=lambda x: x["phi_mean"], reverse=True):
        phi_ci = "$[" + f"{m['phi_ci'][0]:.4f}" + ", " + f"{m['phi_ci'][1]:.4f}" + "]$"
        asr_ci = "$[" + f"{m['asr_ci'][0]:.2f}" + ", " + f"{m['asr_ci'][1]:.2f}" + "]$"
        main.append(
            f"{m['model']} & {m['phi_mean']:.4f} & {phi_ci} & "
    ...
    for p in ordered:
        ...
        dci = "$[" + f"{p['diff_ci'][0]:+.4f}" + ", " + f"{p['diff_ci'][1]:+.4f}" + "]$"
        pw.append(
            f"{p['a_model']} vs.\\ {p['b_model']} & "
            f"{p['mean_diff_phi']:+.4f} & {dci} & "
    ...
    eos = ["% Auto-generated by analyze_stats.py -- EOS completeness vs all samples"]
    for m in sorted(model_stats, key=lambda x: x["phi_mean"], reverse=True):
        co = m["complete_only"]
        if m["n_eos"] == 0:
            phi_eos, lr_eos = "--", "--"
        else:
            phi_eos, lr_eos = f"{co['phi_mean']:.4f}", f"{co['asr']:.2f}"
        eos.append(
            f"{m['model']} & {m['n_eos']} & {m['eos_percent']:.1f} & {m['trunc_percent']:.1f} & "
            f"{m['phi_mean']:.4f} & {phi_eos} & {m['asr']:.2f} & {lr_eos} \\\\"
        )
    (OUT / "tables_eos.tex").write_text("\n".join(eos) + "\n", encoding="utf-8")
...
def _write_topic_table(data: dict[str, dict[int, dict]], common: list[int]) -> None:
    ...
    for label, pretty in TOPIC_MODELS:
        ...
        best_tex = best_t.replace("_", "\\_")
        worst_tex = worst_t.replace("_", "\\_")
        lines.append(
            f"{pretty} & {best_tex} & {topic_mean[best_t]:.4f} & "
            f"{worst_tex} & {topic_mean[worst_t]:.4f} \\\\")
